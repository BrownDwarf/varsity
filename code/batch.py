...
parser = argparse.ArgumentParser(
    prog="batch.py", description="Batch common starfish tasks"
)
parser.add_argument(
    "--run_num", type=int, default=None, help="Which run_number, defaults to run01"
)
parser.add_argument(
    "--convert_fits_to_hdf5_files",
    action="store_true",
    help="Convert csv .dat files to HDF5 files",
)
parser.add_argument(
    "--set_wl_lims",
    action="store_true",
    help="Set the wavelength limits of the spectra",
)
...
parser.add_argument(
    "--train_pca", action="store_true", help="Create, train, and store the PCA emulator"
)
parser.add_argument(
    "--init_run_dirs", action="store_true", help="Initialize output and run directories"
)
parser.add_argument(
    "--generate_s0_o0phi_file", action="store_true", help="Generate the s0_o0phi file"
)
parser.add_argument(
    "--save_csv_spectrum",
    action="store_true",
    help="Save a model spectrum to a csv file",
)
parser.add_argument(
...
parser.add_argument(
    "--run_Starfish", action="store_true", help="Run Starfish spectral inference"
)
parser.add_argument(
    "--qsub_PBS",
    action="store_true",
    help="Write PBS script and run Starfish spectral inference",
)
parser.add_argument(
    "--diagnose",
    action="store_true",
    help="Produce posterior predictive diagnostics after a Starfish run",
)
args = parser.parse_args()

src_dict = {
    "luhman16A_btjd_2285p65": "GS-2021A-DD-104/20210311/reduced/SDC{}_20210311_0025.spec_a0v.fits"
}  # hardcoded for now
...
def generate_s0_o0phi_file(source, order, run_num):
    ...
    file_out = os.path.expandvars(
        "$varsity/sf/{}/{}/output/marley_grid/run{:02d}/"
...
def generate_config_yaml(source, order, run_num=None):
    ...
    config["grid"][
        "hdf5_path"
    ] = "$varsity/sf/{}/m{:03d}/libraries/Bobcat_grid.hdf5".format(source, order)
    ...
    config["grid"]["parrange"] = [
        [int(df[m]["Teff_lo"]), int(df[m]["Teff_hi"])],
        [float(df[m]["logg_lo"]), float(df[m]["logg_hi"])],
    ]
    config["PCA"]["path"] = "$varsity/sf/{}/m{:03d}/libraries/Bobcat_PCA.hdf5".format(
        source, order
    )

    if run_num is None:
        config["Theta_priors"] = "$varsity/sf/{}/m{:03d}/user_prior.py".format(
            source, order
        )
    else:
...
def train_pca(source, order, n_samples=3):
    ...
    cmd2 = (
        "$Starfish/scripts/pca.py --optimize=emcee --samples={} "
...
def run_Starfish(source, order, run_num=1, samples=3, incremental_save=1):
    ...
    os.chdir(run_dir)
    os.system(
        "$varsity/code/star_marley_beta.py "
...
if args.convert_fits_to_hdf5_files:
    for source in sources:
        for band in ["H", "K"]:
            ...
            spec_cln = spec.normalize().remove_nans().trim_edges()
            directory_out = os.path.expandvars("$varsity/data/IGRINS/processed/")
            spec_cln.to_HDF5(directory_out, "luhman16A_btjd_2285p65")
